...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    if is_bot:
        ty += "بوت"
    await msg.reply(f"**» استخـراج الجلسـة **{ty}** بواسطـة T_T_T_U جـارٍ..**.")
    user_id = msg.chat.id
    ...
    if not is_bot:
        ...
        t = "⎆ ** يرجـى إرسـال توكـن بوتـك مثــال ⭐ : 1234567891:SourceT_T_T_U`'**"
    ...
    text = f"**هذا هو {ty} كـود جلسـة** \n\n`{string_session}` \n\n**مستخـرج مـن :** @G4ZBOT\n🍒 **ملاحظـة :** لا تشارك الكود لأحـد لأنـه يستطيع اختراق حسابك من خلالـه قنـاة السورس @T_T_T_U ."
    try:
        if not is_bot:
